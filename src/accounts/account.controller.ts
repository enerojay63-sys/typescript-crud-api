...
function revokeToken(req: any, res: any, next: any) {
    ...
    accountService.revokeToken({ token, ipAddress }).then(() => res.json({ message: 'Token revoked' })).catch(next);
}
...
function getById(req: any, res: any, next: any) {
    if (req.params.id !== req.user.id.toString() && req.user.role !== Role.Admin)
        return res.status(401).json({ message: 'Unauthorized' });
    accountService.getById(req.params.id).then((account: any) => res.json(account)).catch(next);
}
...
function create(req: any, res: any, next: any) {
    accountService.create(req.body).then((account: any) => res.json(account)).catch(next);
}
