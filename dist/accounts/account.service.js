...
async function update(id, params) {
    ...
    if (params.email && account.email !== params.email &&
        await db_1.db.Account.findOne({ where: { email: params.email } })) {
        throw `Email '${params.email}' is already taken`;
    }
    if (params.password)
        params.passwordHash = await bcryptjs_1.default.hash(params.password, 10);
    Object.assign(account, { ...params, updated: new Date() });
    await account.save();
    return basicDetails(account);
}
...
function generateJwtToken(account) {
    return jsonwebtoken_1.default.sign({ sub: account.id, id: account.id }, config_1.config.secret, { expiresIn: '15m' });
}
