...
const account_service_1 = __importDefault(require("../accounts/account.service"));
...
function getById(req, res, next) {
    if (req.params.id !== req.user.id.toString() && req.user.role !== role_1.Role.Admin)
        return res.status(401).json({ message: 'Unauthorized' });
    account_service_1.default.getById(req.params.id).then((account) => res.json(account)).catch(next);
}
...
function create(req, res, next) {
    account_service_1.default.create(req.body).then((account) => res.json(account)).catch(next);
}
